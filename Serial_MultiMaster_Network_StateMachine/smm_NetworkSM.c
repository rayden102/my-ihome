...
void fsm_Idle(void)
{
	...
	if (u16EventFlags & EVENT_RECEIVED_DATA_bm)
	{
		...
		gNSMCurrentState = eSM_StateType.eSM_Receive;
	};
	...
	if (u16EventFlags & EVENT_DATA_READY_TO_SEND_bm)
	{
		...
		gNSMCurrentState = eSM_StateType.eSM_Send;
	};
};
...
void fsm_ProcessData(void)
{
	...
	// If complete than check if frame processing is needed (is it for me?)
		...
		gNSMCurrentState = eSM_StateType.eSM_ExecuteCommand;
...
void fsm_Send(void)
{
	...
	if (eBusyLine.BUSY == gBusyLine)
	{
		gNSMCurrentState = eSM_StateType.eSM_WaitForResend;
	...
	gNSMCurrentState = eSM_StateType.eSM_WaitForResponse;
};
...
void fsm_Retransmission(void)
{
	...
	gNetworkError = eNetworkError.eNE_MaximumRetries;
	gNSMCurrentState = eSM_StateType.eSM_Error;
};
...
void fsm_WaitForResponse(void)
{
	...
	{
		gNSMCurrentState = eSM_StateType.eSM_Retransmission;
...
void fsm_Error(void)
{
	...
	// Clear error
	gNetworkError = eNetworkError.eNE_None;
